...
export class PomodoroDatabase {
  ...
  constructor(dbPath?: string) {
    ...
    this.dbPath =
      dbPath || path.join(home, ".claude", "skills", "pomodoro", "pomodoro.db");
  }
  ...
  getSessionHistory(days: number = 7, limit: number = 100): Session[] {
    ...
    const results = stmt.all(limit) as Session[];
    return results;
  }
  ...
  getStatistics(period: "day" | "week" | "month" | "year" = "week") {
    ...
    return {
      ...
      completionRate: totalSessions
        ...
        : "0",
      totalFocusTimeMinutes: Math.round(totalFocusTimeResult.focusTime || 0),
